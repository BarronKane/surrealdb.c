...
int main(void) {
    ...
    const char* features[][4] = {
        ...
        {"feature:dedicated_support", "Dedicated Support", "24/7 dedicated support team", "enterprise"},
    };
    ...
    UserData users[] = {
        ...
        {"user:carol", "Carol Davis", "carol@techstartup.io", "Lead Developer", "2024-01-20T09:00:00Z"},
        {"user:david", "David Smith", "david@globalcorp.com", "VP Engineering", "2023-06-20T15:00:00Z"},
        {"user:eve", "Eve Johnson", "eve@globalcorp.com", "Security Lead", "2023-07-01T08:30:00Z"},
        {"user:frank", "Frank Miller", "frank@globalcorp.com", "DevOps Engineer", "2023-08-15T10:45:00Z"},
    ...
    for (int i = 0; i < 8; i++) {
        ...
        result = sr_relate(db, &err, &rel_result, memberships[i].user, "member_of", memberships[i].org, &content);
